GLOBAL_VAR_INIT(LOOC_COLOR, "#6699cc")//If this is null, use the CSS for LOOC. Otherwise, use a custom colour.
...
/client/verb/looc(msg as text)
	...
	if(IsGuestKey(key))
		to_chat(src, "<span class='warning'>Guests may not use LOOC.</span>")
		return
	...
	var/message_admin = "<span class='looc'><font color='[GLOB.LOOC_COLOR]'>LOOC: [ADMIN_LOOKUPFLW(mob)]: [msg]</font></span>"
	...
	if(isobserver(mob)) //if you're a spooky ghost
		...
		message_regular = "<span class='looc'><font color='[GLOB.LOOC_COLOR]'>LOOC: [key_to_print]: [msg]</font></span>"
	else
		message_regular = "<span class='looc'><font color='[GLOB.LOOC_COLOR]'>LOOC: [mob.name]: [msg]</font></span>"
